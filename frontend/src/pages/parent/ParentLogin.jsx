...
export default function ParentLogin() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    const result = await login(form.email, form.password, 'parent');
    setLoading(false);
    if (result.success) { notify.success('Welcome!'); navigate('/parent/dashboard'); }
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-green-900 to-green-600 flex items-center justify-center px-4">
      <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-extrabold text-green-800">Parent Login</h1>
          <p className="text-gray-500 text-sm mt-1">Haven of Hope Academy — Parent Portal</p>
        </div>
        {error && (
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div>
            ...
            <input type="email" name="email" value={form.email} onChange={handleChange} placeholder="parent@email.com" required
        ...
        <div className="mt-6 space-y-2 text-center text-sm text-gray-500">
          <p><Link to="/student/login" className="text-green-600 hover:underline font-medium">Student Login</Link>{' · '}<Link to="/teacher/login" className="text-green-600 hover:underline font-medium">Teacher Login</Link></p>
